...
for image in orbit-api nexus-api nexus-web academy edupay-api edupay-web savanex-api savanex-web edusync-api edusync-web; do
...
previous_academy="$(sed -n 's/^ACADEMY_RELEASE=//p' "$ENV_FILE" | tail -n 1)"
...
rollback_on_error() {
  ...
  export ACADEMY_RELEASE="${previous_academy:-$previous}"
