...
docker build --build-arg "VITE_API_BASE_URL=$EDUPAY_API_URL" --build-arg "VITE_RECEIPT_VERIFICATION_BASE_URL=$EDUPAY_PUBLIC_URL" --build-arg "VITE_BASE_PATH=/" -t "kcs/edupay-web:$RELEASE" "$ROOT_DIR/EduPay Smart System/apps/web"
docker build -t "kcs/savanex-api:$RELEASE" "$ROOT_DIR/SAVANEX Project/backend"
docker build --build-arg "VITE_API_URL=$SAVANEX_API_URL" --build-arg "VITE_BASE_PATH=/" -t "kcs/savanex-web:$RELEASE" "$ROOT_DIR/SAVANEX Project/frontend"
