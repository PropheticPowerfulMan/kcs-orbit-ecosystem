...
const aliases = { CD: 'RDC RD Congo DRC Congo-Kinshasa', CG: 'Congo-Brazzaville', US: 'USA United States Amérique', GB: 'UK United Kingdom Royaume-Uni', AE: 'UAE Émirats', KR: 'Corée du Sud', KP: 'Corée du Nord' };
...
export default function InternationalPhoneInput({ value = '', onChange, className = '', disabled, required, 'aria-label': ariaLabel = 'Numéro de téléphone international' }) {
  ...
  return <div className={`international-phone-input ${className}`.trim()} style={{ display: 'grid', gridTemplateColumns: 'clamp(7.75rem, 36%, 10rem) minmax(0, 1fr)', overflow: 'hidden', border: '1px solid rgba(148,163,184,.35)', borderRadius: '.75rem', background: '#ffffff' }}>
    <select title="Choisissez un pays — tapez USA, RDC, UK ou un code pays pour rechercher" aria-label="Pays, drapeau et indicatif" disabled={disabled} value={country} onChange={(event) => { const next = event.target.value; setCountry(next); emit(next, national); }} style={{ minWidth: 0, border: 0, borderRight: '1px solid rgba(148,163,184,.3)', padding: '.75rem', background: '#ffffff', color: '#0f172a', colorScheme: 'light', outline: 'none', fontFamily: "system-ui, 'Segoe UI Emoji', 'Apple Color Emoji', sans-serif", fontWeight: 600, whiteSpace: "nowrap", textOverflow: "ellipsis", overflow: "hidden" }}>{countries.map((item) => <option key={item.country} value={item.country} style={{ background: '#ffffff', color: '#0f172a' }}>{flag(item.country)} {item.name} · {item.country}{item.aliases ? ' · ' + item.aliases : ''} (+{item.callingCode})</option>)}</select>
