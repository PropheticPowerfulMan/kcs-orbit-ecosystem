...
const ResetAccessDialog=({credentials,onClose})=><div className="savanex-modal-backdrop fixed inset-0 z-[9999] grid place-items-center overflow-y-auto p-4" role="dialog" aria-modal="true"><section className="savanex-modal-panel savanex-reset-access-panel w-full max-w-xl p-6"><button type="button" onClick={onClose} className="float-right text-white">Fermer</button><p className="text-xs text-amber-300">RÉINITIALISATION TERMINÉE</p><h3 className="mt-2 text-2xl font-bold text-white">Nouveaux identifiants</h3><div className="mt-6 space-y-3 text-slate-200"><p>Identifiant : <b>{credentials.username}</b></p><p>Code d’accès : <b>{credentials.accessCode}</b></p><p>Mot de passe : <b className="text-emerald-200">{credentials.temporaryPassword}</b></p></div><p className="mt-4 text-xs text-slate-400">Mot de passe à changer à la prochaine connexion.</p></section></div>;
