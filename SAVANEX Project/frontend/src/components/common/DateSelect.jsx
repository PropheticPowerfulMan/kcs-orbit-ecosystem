...
export default function DateSelect({ value = '', onChange, className = '', required, disabled, min, max }) {
  ...
  const emit = (nextYear, nextMonth, nextDay) => {
    ...
    onChange?.({ target: { value: nextYear && nextMonth && nextDay ? `${nextYear}-${nextMonth}-${String(nextDay).padStart(2, '0')}` : '' } });
  };
