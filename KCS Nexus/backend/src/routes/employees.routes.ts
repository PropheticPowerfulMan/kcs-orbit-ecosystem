...
const call=async(path:string,init:RequestInit={})=>{if(!env.SAVANEX_API_URL||!env.SAVANEX_INTELLIGENCE_API_KEY)throw new ApiError(503,'Savanex employee integration is not configured');const c=new AbortController(),t=setTimeout(()=>c.abort(),env.SAVANEX_TIMEOUT_SECONDS*1000);try{const response=await fetch(env.SAVANEX_API_URL.replace(/\/$/,'')+'/api/integration/'+path,{...init,signal:c.signal,headers:{'content-type':'application/json','x-api-key':env.SAVANEX_INTELLIGENCE_API_KEY,...(init.headers||{})}}),body=await response.json().catch(()=>({}));if(!response.ok)throw new ApiError(response.status,body.detail||body.message||'Savanex employee operation failed' );return body}catch(error){if(error instanceof ApiError)throw error;throw new ApiError(502,'Savanex employee service is unavailable')}finally{clearTimeout(t)}}
