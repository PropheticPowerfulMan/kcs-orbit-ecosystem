...
const call=async(path:string,init:RequestInit={})=>{if(!env.SAVANEX_API_URL||!env.SAVANEX_AUTH_API_KEY)throw new ApiError(503,'Savanex employee integration is not configured');const c=new AbortController(),t=setTimeout(()=>c.abort(),env.SAVANEX_TIMEOUT_SECONDS*1000);try{const response=await fetch(env.SAVANEX_API_URL.replace(/\/$/,'')+'/api/integration/'+path,{...init,signal:c.signal,headers:{'content-type':'application/json','x-api-key':env.SAVANEX_AUTH_API_KEY,...(init.headers||{})}}),body=await response.json().catch(()=>({}));if(!response.ok)throw new ApiError(response.status,body.detail||body.message||Object.entries(body).map(([field,value])=>field+': '+(Array.isArray(value)?value.join(', '):String(value))).join(' | ')||'Savanex employee operation failed' );return body}catch(error){if(error instanceof ApiError)throw error;throw new ApiError(502,'Savanex employee service is unavailable')}finally{clearTimeout(t)}}
const compactEmployee=(employee:Record<string,unknown>)=>{const {photo_data,left_fingerprint_data,right_fingerprint_data,user,...rest}=employee;const compactUser=user&&typeof user==='object'?Object.fromEntries(Object.entries(user as Record<string,unknown>).filter(([key])=>!['photo_data','avatar','left_fingerprint_data','right_fingerprint_data'].includes(key))):user;return {...rest,user:compactUser,has_photo:Boolean(photo_data||(user&&typeof user==='object'&&((user as Record<string,unknown>).photo_data||(user as Record<string,unknown>).avatar)))}}
employeesRouter.get('/',asyncHandler(async(_q,res)=>{const employees=await call('employees/');return success(res,Array.isArray(employees)?employees.map(employee=>compactEmployee(employee as Record<string,unknown>)):employees)}))
