...
academyRouter.post("/launch", authenticate, requireRoles("teacher"), asyncHandler(async (req: AuthenticatedRequest, res) => {
  if (!env.KCS_ORBIT_API_URL || !env.KCS_ORBIT_API_KEY || !env.ACADEMY_PUBLIC_URL) throw new ApiError(503, "Academy integration is not configured");
  ...
  const response = await fetch(env.KCS_ORBIT_API_URL.replace(/\/$/, "") + "/api/academy/sso/service-tickets", {
    method: "POST", headers: { "content-type": "application/json", "x-api-key": env.KCS_ORBIT_API_KEY },
