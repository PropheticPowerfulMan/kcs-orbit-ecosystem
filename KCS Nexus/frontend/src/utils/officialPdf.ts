...
const escapeHtml = (value: unknown) => String(value ?? 'â€”').replace(/[&<>"']/g, (character) => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#039;' }[character] ?? character))

/** Opens a print-ready document. Browsers expose â€œSave as PDFâ€ in the native print dialog. */
export const printOfficialPdf = ({ title, subtitle = 'KCS Nexus AI â€” Official school document', metadata = [], columns = [], rows = [], narrative, orientation = 'portrait' }: OfficialPdfOptions) => {
  let fallbackFrame: HTMLIFrameElement | null = null
  let output = window.open('', '_blank', 'width=1180,height=860')
  if (!output) {
    ...
    output = fallbackFrame.contentWindow
  }
