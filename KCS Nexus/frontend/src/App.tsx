    ...
    <main className="mx-auto min-h-[70vh] w-full max-w-3xl px-4 py-12 sm:px-6">
      <section className="overflow-hidden rounded-3xl border border-kcs-gold-300 bg-white shadow-2xl dark:bg-kcs-blue-900">
        ...
        <div className="p-6 sm:p-10">
          <p className="text-xs font-black uppercase tracking-[0.24em] text-kcs-gold-600">KCS Nexus Â· Academic Records</p>
          ...
          {verification?.valid&&<div className="mt-4 rounded-xl bg-emerald-50 p-4 text-sm text-emerald-900"><b>{verification.studentName}</b><p>{verification.studentNumber} Â· {verification.grade}</p><p>Issued: {new Date(verification.issuedAt).toLocaleString()}</p></div>}
          </dl>
    ...
    <>
      ...
      <Suspense fallback={<RouteFallback />}>
        <Routes>
          ...
          <Route path="/course-syllabi" element={<ProtectedRoute allowedRoles={['admin','staff','teacher','student']}><CourseSyllabiPage /></ProtectedRoute>} />
          ...
          <Route element={<Layout />}>
            ...
            />
            {['courses', 'students', 'attendance', 'assignments', 'grades', 'report-card', 'reports', 'diagnostics', 'timetable', 'resources', 'discipline', 'messages', 'settings'].map((segment) => (
              <Route
                key={segment}
