...
export default function InternationalPhoneInput({ value = "", onChange, className = "", disabled, required, "aria-label": ariaLabel = "Numéro de téléphone international" }: InternationalPhoneInputProps) {
  ...
  return <div className={`international-phone-input ${className}`.trim()} style={{ display: "grid", gridTemplateColumns: "minmax(9.5rem, 42%) 1fr", overflow: "hidden", border: "1px solid rgba(148,163,184,.35)", borderRadius: ".75rem", background: "rgba(15,23,42,.55)" }}>
    <select aria-label="Pays et indicatif" disabled={disabled} value={country} onChange={(event) => { const next = event.target.value as CountryCode; setCountry(next); emit(next, national); }} style={{ minWidth: 0, border: 0, borderRight: "1px solid rgba(148,163,184,.3)", padding: ".75rem", background: "transparent", color: "inherit", outline: "none" }}>
      {countries.map((item) => <option key={item.country} value={item.country} style={{ color: "#0f172a" }}>{flag(item.country)} {item.name} (+{item.callingCode})</option>)}
    </select>
