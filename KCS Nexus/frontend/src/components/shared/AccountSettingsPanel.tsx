...
const AccountSettingsPanel = ({ roleLabel }: AccountSettingsPanelProps) => {
  ...
  return (
    <div className="grid gap-6 xl:grid-cols-[0.75fr_1.25fr]">
      ...
      <section className="rounded-2xl border border-gray-100 bg-white p-5 dark:border-kcs-blue-800 dark:bg-kcs-blue-900/50">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><h2 className="font-display text-2xl font-bold text-kcs-blue-900 dark:text-white">Identifiants et profil</h2><p className="mt-1 text-sm text-gray-500 dark:text-gray-400">Modifiez uniquement votre photo et votre mot de passe. Le mot de passe actuel n’est jamais affiché ni récupérable : saisissez-le pour que le serveur vérifie son hash.</p></div><button type="button" disabled={saving} onClick={() => void saveChanges()} className="inline-flex items-center justify-center gap-2 rounded-xl bg-kcs-blue-700 px-4 py-2.5 text-sm font-semibold text-white hover:bg-kcs-blue-800 disabled:opacity-60"><CheckCircle2 size={16}/>{saving ? 'Synchronisation…' : 'Enregistrer'}</button></div>
