...
const buildAdminParentRecordsFromDirectory = (
  ...
  return directory.parents.map((parent) => {
    ...
    return {
      id: parent.id,
      displayId,
      name: [parent.lastName, parent.middleName, parent.firstName].filter(Boolean).join(' ').trim()
        || parent.fullName || 'Parent record pending',
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .badge strong { display: block; color: #75ddf4; font-size: 13px; text-transform: uppercase; }
...
const AdminSectionView = ({
  ...
  useEffect(() => {
    ...
    messagesAPI.getAll({ box: 'sent' }).then((response) => {
      ...
      setCommunicationHistory(rows.map((message: any) => ({
        id: message.id,
        direction: 'Sent',
        audience: message.recipient ? [message.recipient.lastName, message.recipient.middleName, message.recipient.firstName].filter(Boolean).join(' ') + ' (' + message.recipient.role + ')' : message.targetRole ?? 'Audience',
        subject: message.subject,
        body: message.body,
        sender: message.sender ? [message.sender.lastName, message.sender.middleName, message.sender.firstName].filter(Boolean).join(' ') : 'Administration',
  ...
  const transcriptPageSize = 8
  const transcriptPageCount = Math.max(1, Math.ceil(filteredTranscriptStudents.length / transcriptPageSize))
  const paginatedTranscriptStudents = filteredTranscriptStudents.slice((transcriptPage - 1) * transcriptPageSize, transcriptPage * transcriptPageSize)
  useEffect(() => { setTranscriptPage(1) }, [transcriptClassFilter, transcriptQuery])
  useEffect(() => { setTranscriptPage((page) => Math.min(page, transcriptPageCount)) }, [transcriptPageCount])
