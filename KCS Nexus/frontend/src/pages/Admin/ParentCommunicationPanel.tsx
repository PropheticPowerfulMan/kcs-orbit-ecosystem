    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 xl:grid-cols-[1.05fr_.95fr]">
        ...
        <section className="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm dark:border-kcs-blue-800 dark:bg-kcs-blue-900/50">
          ...
          <label className="mt-3 flex cursor-pointer items-center justify-center gap-2 rounded-xl border border-dashed border-kcs-blue-300 p-3 text-sm font-bold text-kcs-blue-700 dark:text-kcs-blue-200"><Paperclip size={18}/>{attachment?.name||c.attachDocument}<input type="file" className="sr-only" accept=".pdf,.doc,.docx,.xls,.xlsx,.txt,.csv,image/jpeg,image/png,image/webp" onChange={event=>{const file=event.target.files?.[0];if(file&&file.size>10*1024*1024){setNotice(c.fileTooLarge);return}setAttachment(file||null)}}/></label>
