...
const isParentIncoming = (message: any) => message.sender?.role === "PARENT"
const messageParentName = (message: any) => isParentIncoming(message) ? displayName(message.sender) : ""
const messageFamilyContext = (message: any) => { const parts = String(message.subject || "").split(" — "); return parts.length > 1 ? parts.at(-1) || "" : "" }
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 xl:grid-cols-[1.05fr_.95fr]">
        ...
        <section className="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm dark:border-kcs-blue-800 dark:bg-kcs-blue-900/50">
          ...
          <label className="mt-3 flex cursor-pointer items-center justify-center gap-2 rounded-xl border border-dashed border-kcs-blue-300 p-3 text-sm font-bold text-kcs-blue-700 dark:text-kcs-blue-200"><Paperclip size={18}/>{attachment?.name||c.attachDocument}<input type="file" className="sr-only" accept=".pdf,.doc,.docx,.xls,.xlsx,.txt,.csv,image/*,audio/*,video/mp4,video/webm,video/quicktime" onChange={event=>{const file=event.target.files?.[0];if(file&&file.size>25*1024*1024){setNotice(c.fileTooLarge);return}setAttachment(file||null)}}/></label><AudioRecorder language={language} disabled={busy} onRecorded={setAttachment}/>
          {attachment?<button type="button" className="mt-2 text-xs font-bold text-red-600" onClick={()=>setAttachment(null)}>{c.removeAttachment}</button>:null}
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[760px] text-sm">
            ...
            <tbody>{historyRows.map((message) => {
              ...
              const fromParent = isParentIncoming(message)
              return <tr key={message.id} className={`border-b transition dark:border-kcs-blue-800 ${selected ? 'bg-kcs-gold-50 shadow-[inset_4px_0_0_#eab308] dark:bg-kcs-gold-900/20' : 'hover:bg-gray-50 dark:hover:bg-kcs-blue-800/30'}`}>
                ...
                <td className="p-3 font-semibold dark:text-white"><span className={`mb-1 inline-flex rounded-full px-2 py-1 text-[10px] font-black uppercase tracking-wide ${fromParent ? "bg-violet-600 text-white" : "bg-sky-100 text-sky-800 dark:bg-sky-900 dark:text-sky-100"}`}>{fromParent ? (language === "fr" ? "Parent vers école" : "Parent to school") : (language === "fr" ? "École vers destinataire" : "School to recipient")}</span><p>{messageRecipient(message)}</p></td>
      ...
      {viewingMessage ? <div className="fixed inset-0 z-[1200] flex items-center justify-center bg-slate-950/80 p-4 backdrop-blur-sm" onClick={() => setViewingMessage(null)} role="dialog" aria-modal="true">
        <section className="max-h-[calc(100vh-2rem)] w-full max-w-3xl overflow-y-auto rounded-3xl border border-sky-200 bg-sky-50 p-6 shadow-2xl dark:border-kcs-blue-700 dark:bg-kcs-blue-950" onClick={(event) => event.stopPropagation()}>
          <div className="flex items-start justify-between gap-4"><div><p className="text-xs font-bold uppercase text-kcs-gold-600">{isParentIncoming(viewingMessage) ? (language === "fr" ? "Message reçu d’un parent" : "Message received from a parent") : c.sentMessage}</p><h3 className="mt-2 text-2xl font-bold text-kcs-blue-950 dark:text-white">{viewingMessage.subject}</h3><p className="mt-1 text-sm text-gray-500 dark:text-gray-300">{messageRecipient(viewingMessage)} · {new Date(viewingMessage.createdAt).toLocaleString(language === 'fr' ? 'fr-FR' : 'en-US')}</p></div><button className={primaryButton} onClick={() => setViewingMessage(null)}><X size={17} />{c.close}</button></div>
          {isParentIncoming(viewingMessage) ? <div className="mt-5 rounded-2xl border border-violet-300 bg-violet-100 p-4 text-violet-950 dark:border-violet-700 dark:bg-violet-950/40 dark:text-violet-100"><p className="font-bold">{language === "fr" ? "Envoyé par le parent :" : "Sent by parent:"} {messageParentName(viewingMessage)}</p>{messageFamilyContext(viewingMessage) ? <p className="mt-1 text-sm">{language === "fr" ? "Élève ou famille concerné(e) :" : "Student or family concerned:"} {messageFamilyContext(viewingMessage)}</p> : null}</div> : null}
          <div className="mt-5 whitespace-pre-wrap rounded-2xl border border-sky-100 bg-sky-100/70 p-5 leading-7 text-kcs-blue-950 dark:border-kcs-blue-800 dark:bg-kcs-blue-900 dark:text-gray-100">{viewingMessage.body}</div>
          ...
          {viewingMessage.hasAttachment?<MessageAttachment message={viewingMessage}/>:null}
        </section>
      </div> : null}
    </div>
  )
