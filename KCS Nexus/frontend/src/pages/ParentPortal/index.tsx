...
import ParentFinancePanel from './ParentFinancePanel'
...
type Assignment = { id: string; status: string; score?: number | null; assignment?: { title?: string; description?: string; dueDate?: string; type?: string; cadence?: string; estimatedMinutes?: number | null; resourceUrl?: string | null; resourceName?: string | null; course?: { name?: string } } }
...
type Contact = { id: string; firstName: string; middleName?: string | null; lastName: string; role: string; accessCode?: string | null; staffProfile?: { function?: string | null; department?: string | null } | null; teacherProfile?: { department?: string | null } | null }
type InternalMessage = { id: string; subject: string; body: string; createdAt: string; readAt?: string | null; senderId: string; recipientId?: string; sender?: Contact; recipient?: Contact; attachmentName?: string | null; attachmentMime?: string | null; attachmentSize?: number | null; hasAttachment?: boolean }
...
const displayDate = (value?: string) => value
  ...
  : '—'
const personName = (person?: Contact) => person ? [person.lastName, person.middleName, person.firstName].filter(Boolean).join(' ') : 'KCS'
const contactRoleLabel = (person?: Contact, language: "fr" | "en" = "fr") => {
  if (!person) return language === "fr" ? "École KCS" : "KCS School"
  if (person.accessCode === "ACC-ADM-SUPER1") return language === "fr" ? "Super Administrateur — Direction générale" : "Super Administrator — Executive leadership"
  if (person.role === "ADMIN") return `${language === "fr" ? "Administrateur" : "Administrator"} — ${person.staffProfile?.function || person.staffProfile?.department || (language === "fr" ? "Administration scolaire" : "School administration")}`
  if (person.role === "STAFF") return `${language === "fr" ? "Personnel administratif" : "Administrative staff"} — ${person.staffProfile?.function || person.staffProfile?.department || (language === "fr" ? "Service scolaire" : "School service")}`
  if (person.role === "TEACHER") return `${language === "fr" ? "Enseignant" : "Teacher"}${person.teacherProfile?.department ? ` — ${person.teacherProfile.department}` : ""}`
  return person.role
}
const personLabel = (person?: Contact) => person ? `${personName(person)} — ${contactRoleLabel(person)}` : "KCS — École"
...
export default function ParentPortal() {
  ...
  const parentDisplayName = [user?.lastName, user?.middleName, user?.firstName].filter(Boolean).join(' ') || 'KCS Parent'
    ...
    Promise.allSettled([studentsAPI.getMyChildren(), eventsAPI.getAll(), notificationsAPI.getAll(), messagesAPI.getAll(), messagesAPI.getContacts()])
      .then((results) => {
        ...
        const loaded = (Array.isArray(childPayload) ? childPayload : []).map((profile: any): ParentChild => ({
          ...
          gpa: profile.gpa ?? null, attendance: profile.attendanceRate ?? null, academicSummary: profile.academicSummary, academicAccess: profile.academicAccess,
        }))
        setChildren(loaded)
        ...
        if (results[4].status === 'fulfilled') {
          ...
          setContacts(values)
          const officialContact = values.find((contact: Contact) => contact.role === 'ADMIN') ?? values[0]
          setRecipientId(officialContact?.id ?? '')
          setRecipientRole(officialContact?.role ?? '')
        }
  ...
  useEffect(() => {
    if (segment !== 'messages') return
    const unreadReceived = messages.filter((message) => message.recipientId === user?.id && !message.readAt)
    if (!unreadReceived.length) return
    setMessages((items) => items.map((message) => unreadReceived.some((unreadMessage) => unreadMessage.id === message.id) ? { ...message, readAt: new Date().toISOString() } : message))
    void Promise.allSettled(unreadReceived.map((message) => messagesAPI.markRead(message.id)))
  }, [segment, messages, user?.id])
  ...
  const filteredMessages = useMemo(() => {
    const words = messageQuery.toLowerCase().trim().split(/\s+/).filter(Boolean)
    return messages.filter((item) => {
      const outgoing = item.senderId === user?.id
      const contact = outgoing ? item.recipient : item.sender
      const searchable = [item.subject, item.body, personLabel(contact)].join(' ').toLowerCase()
      return words.every((word) => searchable.includes(word)) && (!messageFrom || item.createdAt.slice(0, 10) >= messageFrom) && (!messageTo || item.createdAt.slice(0, 10) <= messageTo) && (messageDirection === 'all' || (messageDirection === 'sent') === outgoing) && (!messageContact || contact?.id === messageContact)
    })
  }, [messages, messageQuery, messageFrom, messageTo, messageDirection, messageContact, user?.id])
  const contactRoles = useMemo(() => [...new Set(contacts.map((contact) => contact.role))].sort(), [contacts])

  const sendMessage = async () => {
    if (!recipientId || subject.trim().length < 2 || !messageBody.trim()) return
    setSending(true); setFeedback('')
    try {
      const payload = new FormData(); payload.append("recipientId", recipientId); payload.append("subject", subject.trim() + " - " + (selectedChild?.name ?? "Family")); payload.append("body", messageBody.trim()); if (messageAttachment) payload.append("attachment", messageAttachment); const response = await messagesAPI.send(payload)
      setMessages((items) => [response.data.data, ...items]); setSubject(''); setMessageBody(''); setMessageAttachment(null); setFeedback('Message sent and saved in your family history.')
  ...
  const titles: Record<string, string> = language === "fr" ? { dashboard: "Tableau de bord familial", performance: "R\u00e9sultats scolaires", grades: "Notes et bulletins", notifications: "Notifications", messages: "Messages de l\u0027ecole", calendar: "Calendrier scolaire", finance: "Finances et EduPay", profile: "Profil familial", settings: "Param\u00e8tres du compte" } : { dashboard: "Family Dashboard", performance: "Academic Performance", grades: "Grades & Reports", notifications: "Notifications", messages: "School Messages", calendar: "School Calendar", finance: "Finance & EduPay", profile: "Family Profile", settings: "Account Settings" }
  const title = titles[segment] ?? (language === "fr" ? "Portail parent" : "Parent Portal")
  ...
  const academics = academicLocked ? financialLockCard : (
    <div className="space-y-6">
      ...
      <div className={card}><h2 className="mb-4 font-bold text-kcs-blue-900 dark:text-white">Assignments</h2>{assignments.length === 0 ? empty('No assignment is currently linked to this student.') : <div className="grid gap-3 md:grid-cols-2">{assignments.map((item) => <div key={item.id} className="rounded-xl bg-gray-50 p-4 dark:bg-kcs-blue-800/30"><p className="font-semibold dark:text-white">{item.assignment?.title ?? 'Assignment'}</p><p className="mt-1 text-sm text-gray-500">{item.assignment?.course?.name ?? 'Course'} · due {displayDate(item.assignment?.dueDate)}</p><p className="mt-2 text-sm text-gray-600 dark:text-gray-300">{item.assignment?.description || 'No additional teacher instructions.'}</p><div className="mt-2 flex flex-wrap gap-2 text-xs font-bold text-kcs-blue-700"><span className="rounded-full bg-kcs-blue-100 px-2 py-1">{item.assignment?.type || 'WORK'}</span><span className="rounded-full bg-kcs-blue-100 px-2 py-1">{item.assignment?.cadence?.replace('_',' ') || 'ONE TIME'}</span>{item.assignment?.estimatedMinutes&&<span className="rounded-full bg-kcs-blue-100 px-2 py-1">{item.assignment.estimatedMinutes} min</span>}<span className="rounded-full bg-kcs-blue-100 px-2 py-1">{item.status}</span></div>{item.assignment?.resourceUrl&&<a className="mt-3 inline-block text-sm font-bold text-kcs-blue-600 underline" href={item.assignment.resourceUrl} target="_blank" rel="noreferrer">{item.assignment.resourceName||'Open teacher resource'}</a>}</div>)}</div>}</div>
      ...
      <OfficialTranscriptPanel studentId={selectedChild?.localProfileId ?? selectedChild?.id} />
    </div>
  )
  ...
  const content = segment === 'settings' ? <AccountSettingsPanel roleLabel="Parent account" /> : segment === 'performance' || segment === 'grades' ? academics : segment === 'notifications' ? <ParentNotificationsPanel notices={notices} onChange={setNotices} /> : segment === 'messages' ? (
    <div className="grid gap-6 xl:grid-cols-2"><div className={card}><h2 className="mb-4 font-bold text-kcs-blue-900 dark:text-white">Conversation history</h2><div className="mb-4 grid gap-2 sm:grid-cols-2"><input className={field+" sm:col-span-2"} value={messageQuery} onChange={e=>setMessageQuery(e.target.value)} placeholder="Sujet, contenu, nom ou rôle…"/><label className="text-xs text-gray-500">Du<input type="date" className={field+" mt-1"} value={messageFrom} onChange={e=>setMessageFrom(e.target.value)}/></label><label className="text-xs text-gray-500">Au<input type="date" min={messageFrom||undefined} className={field+" mt-1"} value={messageTo} onChange={e=>setMessageTo(e.target.value)}/></label><select className={field} value={messageDirection} onChange={e=>setMessageDirection(e.target.value)}><option value="all">Tous les échanges</option><option value="received">Reçus</option><option value="sent">Envoyés</option></select><select className={field} value={messageContact} onChange={e=>setMessageContact(e.target.value)}><option value="">Tous les correspondants</option>{contacts.map(c=><option key={c.id} value={c.id}>{personLabel(c)}</option>)}</select></div>{filteredMessages.length === 0 ? empty('Aucun message ne correspond à ces critères.') : <div className="max-h-[62vh] space-y-3 overflow-y-auto">{filteredMessages.map((item) => <div key={item.id} className="rounded-xl bg-gray-50 p-4 dark:bg-kcs-blue-800/30"><div className="flex justify-between gap-3"><p className="font-semibold dark:text-white">{item.subject}</p><span className="text-xs text-gray-400">{displayDate(item.createdAt)}</span></div><p className="mt-1 text-xs font-semibold text-kcs-blue-600">{item.senderId === user?.id ? `To ${personLabel(item.recipient)}` : `From ${personLabel(item.sender)}`}</p><p className="mt-2 whitespace-pre-wrap text-sm text-gray-600 dark:text-gray-300">{item.body}</p><MessageAttachment message={item} compact/>{item.senderId!==user?.id&&item.sender?.id&&<button type="button" onClick={()=>{setRecipientId(item.sender!.id);setRecipientRole(item.sender!.role);setSubject('Re: '+item.subject);setMessageBody('');setFeedback('Réponse préparée pour '+personLabel(item.sender))}} className="mt-3 rounded-lg bg-kcs-blue-700 px-3 py-2 text-xs font-bold text-white">Répondre dans Nexus</button>}</div>)}</div>}</div><div className={card}><div className="mb-4 rounded-xl border border-sky-200 bg-sky-50 p-3 text-sm text-kcs-blue-900 dark:border-sky-700 dark:bg-kcs-blue-800 dark:text-white"><b>Canal direct Parent – Super Administration</b><p className="mt-1 text-xs">Chaque message et chaque réponse restent enregistrés dans votre historique sécurisé Nexus.</p></div><h2 className="mb-4 font-bold text-kcs-blue-900 dark:text-white">Write to the school</h2><div className="space-y-3"><select className={field} value={recipientRole} onChange={(event) => { setRecipientRole(event.target.value); setRecipientId("") }}><option value="">All school services and roles</option>{contactRoles.map((role) => <option key={role} value={role}>{role === "ADMIN" ? (language === "fr" ? "Direction et administrateurs" : "Leadership and administrators") : role === "STAFF" ? (language === "fr" ? "Personnel administratif" : "Administrative staff") : role === "TEACHER" ? (language === "fr" ? "Enseignants" : "Teachers") : role}</option>)}</select><select className={field} value={recipientId} onChange={(event) => setRecipientId(event.target.value)}><option value="">Select an authorized contact</option>{contacts.filter((contact) => !recipientRole || contact.role === recipientRole).map((contact) => <option key={contact.id} value={contact.id}>{personName(contact)} — {contactRoleLabel(contact, language)}</option>)}</select><input className={field} value={subject} onChange={(event) => setSubject(event.target.value)} placeholder="Subject" maxLength={160}/><textarea className={`${field} min-h-36`} value={messageBody} onChange={(event) => setMessageBody(event.target.value)} placeholder={`Message regarding ${selectedChild?.name ?? 'your family'}`} maxLength={10000}/><label className="flex cursor-pointer items-center justify-center rounded-xl border border-dashed border-kcs-blue-300 p-3 text-sm font-semibold dark:text-sky-200">{messageAttachment?.name || (language === "fr" ? "Joindre un audio, une vidéo, une image ou un document" : "Attach audio, video, image or document")}<input type="file" className="sr-only" accept="image/*,audio/*,video/mp4,video/webm,video/quicktime,.pdf,.doc,.docx,.xls,.xlsx,.txt,.csv" onChange={(event)=>{const file=event.target.files?.[0]||null;if(file&&file.size>25*1024*1024){setFeedback(language === "fr" ? "Le fichier ne doit pas dépasser 25 Mo." : "The file must not exceed 25 MB.");return}setMessageAttachment(file)}}/></label><AudioRecorder language={language} disabled={sending} onRecorded={setMessageAttachment}/><button className={`${button} w-full`} disabled={sending || !recipientId || subject.trim().length < 2 || !messageBody.trim()} onClick={() => void sendMessage()}>{sending ? 'Sending…' : 'Send securely'}</button>{feedback && <p className="rounded-xl bg-kcs-blue-50 p-3 text-sm text-kcs-blue-800 dark:bg-kcs-blue-800 dark:text-white">{feedback}</p>}</div></div></div>
  ...
  : segment === 'finance' ? <ParentFinancePanel />
  : segment === 'profile' ? <div className="grid gap-6 xl:grid-cols-2"><div className={card}><div className="flex items-center gap-4"><div className="flex h-20 w-20 items-center justify-center rounded-2xl bg-kcs-blue-100 text-xl font-bold text-kcs-blue-700">{user?.firstName?.[0]}{user?.lastName?.[0]}</div><div><h2 className="text-2xl font-bold dark:text-white">{parentDisplayName}</h2><p className="text-sm text-gray-500">Authorized family account</p></div></div><div className="mt-5 space-y-2 text-sm"><a className="flex items-center gap-2 text-kcs-blue-700 dark:text-kcs-blue-300" href={`mailto:${user?.email}`}><Mail size={16}/>{user?.email}</a>{user?.phone && <a className="flex items-center gap-2 text-kcs-blue-700 dark:text-kcs-blue-300" href={`tel:${user.phone}`}><Phone size={16}/>{user.phone}</a>}</div></div><div className={card}><h2 className="mb-4 font-bold dark:text-white">Children linked to this account</h2><div className="space-y-3">{children.map((child) => <button key={child.id} onClick={() => setSelectedId(child.id)} className="flex w-full items-center justify-between rounded-xl bg-gray-50 p-4 text-left dark:bg-kcs-blue-800/30"><span><strong className="block dark:text-white">{child.name}</strong><span className="text-sm text-gray-500">{child.grade} · {child.studentNumber}</span></span><CheckCircle2 className="text-green-500" size={20}/></button>)}</div></div></div>
  ...
  const overviewDescription = language === 'fr' ? 'Une vue familiale sécurisée des résultats, présences, finances et communications scolaires.' : 'A secure family view of academics, attendance, finance, and school communications.'

  return <div className="portal-shell parent-portal-shell flex min-h-screen bg-sky-50 dark:bg-kcs-blue-950"><PortalSidebar badges={{ '/portal/parent/messages': unreadMessages }}/><main className="min-h-0 min-w-0 flex-1 overflow-x-hidden overflow-y-auto"><div className="portal-dashboard-topbar sticky top-0 z-20 border-b px-4 py-3 backdrop-blur-2xl sm:px-6 sm:py-4"><div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between"><div className="min-w-0"><h1 className="portal-dashboard-title font-display text-xl font-bold leading-tight sm:text-2xl">{getLocalizedGreeting(language)}{user?.firstName ? `, ${user.firstName}` : ''}</h1><p className="mt-1 text-sm font-medium text-kcs-blue-700 dark:text-kcs-blue-100">{getLocalizedPortalDate(language)} - {overviewDescription}</p></div>{children.length > 0 && <label className="min-w-64 text-xs font-bold uppercase text-kcs-blue-700 dark:text-kcs-blue-200">{language === 'fr' ? 'Élève actif' : 'Active child'}<select className={`${field} mt-1 normal-case`} value={selectedChild?.id ?? ''} onChange={(event) => setSelectedId(event.target.value)}>{children.map((child) => <option key={child.id} value={child.id}>{child.name} - {child.grade}</option>)}</select></label>}</div></div><div className="w-full max-w-none p-4 sm:p-6 lg:p-8">{segment !== 'dashboard' && <h2 className="mb-6 font-display text-3xl font-bold text-kcs-blue-950 dark:text-white">{title}</h2>}{error && <p className="mb-5 rounded-xl bg-red-50 p-4 text-sm font-semibold text-red-700">{error}</p>}{!loading && academicAccess?.exempt && <p className="mb-5 rounded-xl border border-emerald-200 bg-emerald-50 p-3 text-sm font-bold text-emerald-800 dark:border-emerald-700 dark:bg-emerald-950/30 dark:text-emerald-200">Accès académique autorisé selon la politique institutionnelle.</p>}{!loading && academicLocked && segment === 'dashboard' ? <div className="mb-6">{financialLockCard}</div> : null}{loading ? <div className={card}><div className="flex items-center gap-3 text-gray-500"><Clock className="animate-spin" size={20}/>Loading the secure family workspace…</div></div> : children.length === 0 ? <div className={card}><UserRound className="text-kcs-blue-500"/><h2 className="mt-3 text-xl font-bold dark:text-white">No child is linked to this parent account</h2><p className="mt-2 text-sm text-gray-500">Please contact the school registry. The portal does not infer or expose unverified family links.</p></div> : content}<div className="mt-8"><SuggestionBox/></div></div></main></div>
}
