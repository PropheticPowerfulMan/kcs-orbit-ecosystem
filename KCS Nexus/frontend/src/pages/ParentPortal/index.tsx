    ...
    Promise.allSettled([studentsAPI.getMyChildren(), eventsAPI.getAll(), notificationsAPI.getAll(), messagesAPI.getAll(), messagesAPI.getContacts()])
      .then((results) => {
        ...
        const loaded = (Array.isArray(childPayload) ? childPayload : []).map((profile: any): ParentChild => ({
          ...
          gpa: profile.gpa == null ? null : Number(profile.gpa), attendance: profile.attendanceRate == null ? null : Number(profile.attendanceRate),
        }))
        setChildren(loaded)
  ...
  const academics = (
    <div className="space-y-6">
      <div className="grid gap-4 sm:grid-cols-3">
        {[['GPA', selectedChild?.gpa ? selectedChild.gpa.toFixed(2) : '—'], ['Attendance', selectedChild?.attendance != null ? `${selectedChild.attendance}%` : 'Aucune donnée'], ['Recorded grades', String(grades.length)]].map(([label, value]) => <div key={label} className={card}><p className="text-xs font-bold uppercase tracking-wide text-gray-400">{label}</p><p className="mt-2 text-3xl font-bold text-kcs-blue-900 dark:text-white">{value}</p></div>)}
      </div>
      <div className={card}>
