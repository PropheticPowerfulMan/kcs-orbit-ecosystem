...
const StaffPortal = () => {
  ...
  const roleTitle = language === 'fr' ? 'Personnel administratif' : 'Administrative Staff'
  const workspaceTitle = language === 'fr' ? 'Espace du personnel administratif' : 'Administrative Staff workspace'
  ...
  const permissionsView = <div className="grid gap-6 xl:grid-cols-2"><section className={card}><h2 className="font-bold dark:text-white">{language === 'fr' ? 'Autorisations du personnel administratif' : 'Administrative Staff permissions'}</h2><div className="mt-4 flex flex-wrap gap-2">{['Read shared records','Process admission stages','Read EduPay summary','Send internal messages','Read report workflow','No final approval','No account provisioning'].map((item)=><span key={item} className="rounded-full bg-kcs-blue-50 px-3 py-1 text-xs font-bold text-kcs-blue-700">{item}</span>)}</div></section><section className={card}><h2 className="font-bold dark:text-white">My audit activity</h2>{(overview.recentActivity??[]).length===0?empty('No recent administrative action recorded for this account.'):<div className="mt-4 space-y-3">{overview.recentActivity.map((item:any)=><div key={item.id} className="rounded-xl bg-gray-50 p-3 text-sm dark:bg-kcs-blue-800/30"><b>{item.action}</b><p className="text-xs text-gray-500">{item.targetType} · {new Date(item.createdAt).toLocaleString()}</p></div>)}</div>}</section></div>
  ...
  return <div className="portal-shell flex"><PortalSidebar/><main><header className="portal-dashboard-topbar sticky top-0 z-20 border-b px-4 py-3 backdrop-blur-2xl sm:px-6 sm:py-4"><div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between"><div><h1 className="portal-dashboard-title font-display text-xl font-bold sm:text-2xl">{getLocalizedGreeting(language)}, {user?.firstName}</h1><p className="mt-1 text-sm font-medium text-kcs-blue-700 dark:text-kcs-blue-100">{getLocalizedPortalDate(language)} · {workspaceTitle} · {status}</p></div><div className="flex gap-2"><Link to="/portal/staff/announcements" className="btn-primary flex items-center gap-2 py-2 text-sm"><Megaphone size={16}/>Announcements</Link><Link to="/portal/staff/messages" className="btn-gold flex items-center gap-2 py-2 text-sm"><MessageSquare size={16}/>Messages</Link></div></div></header><div className="space-y-6 p-4 sm:p-6">{notice&&<p className="rounded-xl bg-kcs-blue-50 p-3 text-sm font-semibold text-kcs-blue-800">{notice}</p>}{status==='error'&&<button onClick={()=>void load()} className={button}>Retry synchronization</button>}{content}</div></main></div>
}
