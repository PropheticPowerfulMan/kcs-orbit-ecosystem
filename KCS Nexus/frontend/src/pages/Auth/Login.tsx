...
const loginSchema = z.object({
  email: z.string().min(1, 'Email or access code is required'),
            ...
            <div className="mx-auto max-w-md">
              ...
              {isAuthenticated && user && (
                <div className="mb-6 rounded-2xl border border-kcs-blue-200 bg-kcs-blue-50 px-4 py-3 text-sm text-kcs-blue-800 dark:border-kcs-blue-800 dark:bg-kcs-blue-900/30 dark:text-kcs-blue-200">
                  Active session: {user.firstName} {user.lastName} ({user.role}). Choose a demo account below to replace this session.
                </div>
              )}

              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                <div>
                  <label className="mb-1.5 block text-xs font-semibold text-gray-600 dark:text-gray-300">Email or access code</label>
                  <div className="relative">
                    <Mail size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400" />
                    <input {...form.register('email')} className="input-kcs pl-11" placeholder="name@kcsnexus.edu or ACC-ADM-SUPER1" />
                  </div>
