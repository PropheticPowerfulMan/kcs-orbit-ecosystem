        ...
        <div className="grid w-full max-w-6xl overflow-hidden rounded-[2rem] border border-white/10 bg-white/5 shadow-2xl backdrop-blur-xl lg:grid-cols-[0.9fr_1.1fr]">
          <aside className="border-b border-white/10 p-7 text-white lg:border-b-0 lg:border-r lg:p-10">
            ...
            <div className="mt-8 space-y-3">
              ...
              {administrativeIdentifiers.map((account) => (
          ...
          <motion.main initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.45 }} className="bg-white p-7 dark:bg-kcs-blue-950/95 sm:p-10">
            <div className="mx-auto max-w-md">
              ...
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                <label className="block"><span className="mb-1.5 block text-xs font-semibold text-gray-600 dark:text-gray-300">E-mail ou code d’accès</span><span className="relative block"><Mail size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400"/><input {...form.register('email')} className="input-kcs pl-11" autoComplete="username" placeholder="nom@ecole.cd ou ACC-..."/></span>{form.formState.errors.email && <span className="mt-1 block text-xs text-red-500">{form.formState.errors.email.message}</span>}</label>
                <label className="block"><span className="mb-1.5 block text-xs font-semibold text-gray-600 dark:text-gray-300">Mot de passe</span><span className="relative block"><Lock size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400"/><input {...form.register('password')} type={showPassword?'text':'password'} className="input-kcs pl-11 pr-11" autoComplete="current-password" placeholder="Votre mot de passe"/><button type="button" onClick={()=>setShowPassword((value)=>!value)} className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400" aria-label={showPassword?'Masquer':'Afficher'}>{showPassword?<EyeOff size={16}/>:<Eye size={16}/>}</button></span>{form.formState.errors.password && <span className="mt-1 block text-xs text-red-500">{form.formState.errors.password.message}</span>}</label>
                ...
                {recoveryOpen && <div className="rounded-2xl border border-kcs-blue-200 bg-kcs-blue-50 p-4 dark:border-kcs-blue-800 dark:bg-kcs-blue-900/30"><p className="text-sm font-semibold text-kcs-blue-900 dark:text-white">Récupération sécurisée</p>{!resetToken?<div className="mt-3 space-y-3"><input type="email" value={recoveryEmail} onChange={(event)=>setRecoveryEmail(event.target.value)} className="input-kcs" placeholder="Adresse e-mail du compte"/><button type="button" onClick={()=>void requestRecovery()} disabled={isLoading} className="btn-secondary w-full disabled:opacity-60">Envoyer le lien</button></div>:<div className="mt-3 space-y-3"><input type="password" value={newPassword} onChange={(event)=>setNewPassword(event.target.value)} className="input-kcs" placeholder="Nouveau mot de passe" minLength={8}/><button type="button" onClick={()=>void completeRecovery()} disabled={isLoading||newPassword.length<8} className="btn-secondary w-full disabled:opacity-60">Réinitialiser</button></div>}{recoveryMessage&&<p className="mt-3 text-sm text-emerald-600">{recoveryMessage}</p>}{recoveryError&&<p className="mt-3 text-sm text-red-600">{recoveryError}</p>}</div>}
